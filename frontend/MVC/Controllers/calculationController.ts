...
export class api {
    static async sendCalculationToAPI(weight: number, dailyDoseNumber: number, basalDoseNumber: number, carbDose: number, carbsDoseNumber: number, userID: number): Promise < boolean > {
        const date = new Date().toLocaleString();
        const json = JSON.stringify({"weight":weight, "dailyDoseNumber":dailyDoseNumber, "basalDoseNumber":basalDoseNumber, "carbDose":carbDose, "carbsDoseNumber":carbsDoseNumber, "calculationDateTime":date, "user": {"id": userID }});
